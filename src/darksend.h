...
{
public:
    /// Is the input associated with this public key? (and there is 5000 NODELAB - checking if valid masternode)
...
private:
    ...
    /// Get the denominations for a specific amount of nodelab.
    ...
    bool Prepanodelabominate(int nMinRounds, int nMaxRounds, std::string& strErrorRet, std::vector<CTxIn>& vecTxInRet, std::vector<CTxOut>& vecTxOutRet);
