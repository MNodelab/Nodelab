...
{
    if(fLiteMode) return; // ignore all NodeLab related functionality
...
/*
    ...
    allow endless transaction that would bloat NodeLab and make it unusable. To
...
bool CDarksendPool::SubmitDenominate()
{
    ...
    for(int i = 0; i < nPrivateSendRounds; i++) {
        if(Prepanodelabominate(i, i+1, strError, vecTxInRet, vecTxOutRet)) {
    ...
    if(Prepanodelabominate(0, nPrivateSendRounds, strError, vecTxInRet, vecTxOutRet)) {
...
bool CDarksendPool::Prepanodelabominate(int nMinRounds, int nMaxRounds, std::string& strErrorRet, std::vector<CTxIn>& vecTxInRet, std::vector<CTxOut>& vecTxOutRet)
{
    ...
    */
    ...
    LogPrintf("CDarksendPool::Prepanodelabominate -- max value: %f\n", (double)nValueIn/COIN);
...
{
    if(fLiteMode) return; // disable all NodeLab specific functionality
    ...
    RenameThread("nodelab-privatesend");
