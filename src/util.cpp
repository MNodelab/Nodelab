...
const char * const BITCOIN_CONF_FILENAME = "nodelab.conf";
const char * const BITCOIN_PID_FILENAME = "nodelabd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nodelab" is a composite category enabling all NodeLab-related debug output
            if(ptrCategory->count(string("nodelab"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NodeLabCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NodeLabCore
    // Mac: ~/Library/Application Support/NodeLabCore
    // Unix: ~/.nodelabcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NodeLabCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NodeLabCore";
#else
    // Unix
    return pathRet / ".nodelabcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nodelab.conf if it does not excist
        ...
        if (configFile != NULL) {
        	...
        	LogPrintf("nodelab.conf file not found or can't be created\n");
    ...
    {
        // Don't overwrite existing settings so command line settings override nodelab.conf
...
void AddSeedsToConfigFile(FILE* configFile) {
	...
	fprintf(configFile,"addnode=185.246.152.210:14069\r\n");
	fprintf(configFile,"addnode=185.246.153.85:14069\r\n");
	fprintf(configFile,"addnode=185.246.153.225:14069\r\n");
	fprintf(configFile,"addnode=185.246.153.224:14069\r\n");
}
